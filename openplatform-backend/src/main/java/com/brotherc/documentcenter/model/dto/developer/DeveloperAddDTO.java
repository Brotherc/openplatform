...
@Data
@Schema(description = "开发者创建入参")
public class DeveloperAddDTO {

    @NotBlank(message = "用户名不能为空")
    @Size(min = 3, max = 50, message = "用户名长度必须在3-50个字符之间")
    @Pattern(regexp = "^[a-zA-Z0-9_]+$", message = "用户名只能包含字母、数字和下划线")
    @Schema(description = "用户名（3-50个字符，只能包含字母、数字和下划线）", requiredMode = Schema.RequiredMode.REQUIRED)
    ...
    @Schema(description = "密码（6-20个字符，必须包含大小写字母和数字）", requiredMode = Schema.RequiredMode.REQUIRED)
